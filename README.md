...
- **Ranking is fetched on demand** — the panel loads the ranking when it is opened (and on refresh), so a badge that stays closed never pays for the all-session scan. The parenthesized today share on the 本会话花费 row rides that same fetch, so it costs no extra request (no parentheses until the ranking settles, and up to 60 seconds behind afterwards).
