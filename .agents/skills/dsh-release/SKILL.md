---
name: dsh-release
description: 按 DeepSeek Harness 官方 GitHub Release 格式发布 @rayadesu 计费插件的 npm 包与 GitHub Release；默认只做本地 pack 安装交给用户重启验证，用户明确说「发布」后才提交推送、发 release、发 npm
...
**两阶段铁律：阶段 A（改代码 → 本地 pack 安装 → 用户重启验证）做完后，绝不提交、绝不推送、绝不打 tag、绝不发布 npm/GitHub。只有用户明确说「发布」才进入阶段 B。**

## 阶段 A — 改动 + 本地 pack 安装（默认执行）
...
5. **装入 web profile（pnpm 11 陷阱：同名同版本 tarball 视为未变，必须先 remove 再 add）**：
...
6. **交给用户**：提示「请重启 `dsh web` 并硬刷新验证」，列出本轮应验证的行为点。
7. **等待用户说出「发布」**（或提出新改动）。用户没发话，到此为止。
...
3. **中文提交并推送**：`git add -A` → 中文提交信息（`fix:` / `feat:` / `docs:` / `chore:` …，按仓库既有风格带 `(vX.Y.Z)`）→ `git push origin main`。
