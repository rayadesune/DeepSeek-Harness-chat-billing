...
| --- | --- | --- |
| 轻量 | **≤ 8** | 定位 1–2 + 改动 1–2 + 受影响用例 1 + `local-install` 1 + 文档 1–2 |
| 常规 | **≤ 12** | 轻量 + 官方规则核实 1–2 + 二次用例 1 |
| 完整 | **≤ 20** | 含 `test`/`build`/`verify` 三连与提交分组 |

超预算就**停下合并步骤**：能一条 pwsh 跑完的不要拆成三条，互不依赖的读 / 查 / 改**塞进同一条消息**一次发完，别一轮一个。实测（`DSH_HOME/sessions` 会话日志）：上一轮「今日 Token 加命中率括号」= **58 次请求 ≈ 6.0M token**，而输出只占其中几 K，钱全花在「每次请求重放整段上下文」上；本轮把同样量级的动作并成 4 轮发，只花了 **0.6M 量级**。那一轮的浪费主要是：反复精读同一批文件、把 build / pack / remove / add 拆开单跑、以及一条同步命令跑满 300s 超时后的排查。

### 1. 改动定档（先判断，再动手）

| 档位 | 典型改动 | 必跑校验 | 打包 / 安装 |
| --- | --- | --- | --- |
| 轻量 | 文案、locale 键、CSS、注释、单点样式 | 只跑受影响的 spec（`pnpm exec vitest run <spec>`，或给 `pnpm run test` 加 `-- --reporter=dot`） | 只重打**改动的那一个包**，只 remove/add 那一个 |
| 常规 | 单包源码逻辑 | 该包 spec + `pnpm run build:host` / `pnpm run build:client`（**动了 Remote 类型或 typert 产物必须先 `build:host`**，见 AGENTS.md 的构建顺序硬约束） | 同上 |
...
- **巨型单行文件不要 `read`**：README 的面板段落是 8–10K 字符的**一行**，`offset`/`limit` 也切不开它。fs 策略要求「先读后改」，这类文件的锚点替换改用一条 `node -e` 脚本做（只回 `ok` / `BAD` 一行）——比把整段抄进上下文便宜一个数量级。
...
- **一条命令收尾**（构建 → 打包 → 装 profile → 抽查产物），不要拆成 4–6 次工具调用：
  ...
  脚本只回 4–6 行（每步一行 + 抽查计数），**失败才**附上那一步的尾巴；`--face host|both` 选编译面，`--dry-run` 只看计划，`--no-build` 跳过构建。它内部就是「重装只针对改动包：`remove <pkg>` + `add <tgz>` 单个包」（pnpm 11 的同名同版本陷阱只影响被替换的那个包），并把 `remove` 失败降级成跳过（包本来就不在 profile 里也照样装得上）。
- **别再手写 remove / add 两条命令同步等**：上一轮这两条同步跑满 300s 超时、还让 client 包从 profile 里短暂消失，随后的排查又烧掉 5 次请求。长命令一律 `run_in_background`，再用 `job_output` 收一次。
...
3. **构建 + 打包 + 装 profile：一条命令**（`lib/` 是 pack 的产物，改动生效必须重新构建）。`scripts/local-install.mjs` 内部按顺序做完四件事：`pnpm run build:<face>` → `npm pack` 到 `$DSH_HOME\local-tarballs`（**不 bump 版本号**）→ `dsh plugin --profile web remove/add` **只处理改动的那一个包** → 在**装好的** `lib/` 里数 `--check` 字符串。
   ...
   编译面：改 client 面 `--face client`、改宿主面 `--face host`、跨包或拿不准 `--face both`（**动了 Remote 类型或 typert 产物先 host**，见 AGENTS.md 的构建顺序硬约束）。三包都改才并列三个包名。**长命令放后台**（`run_in_background`）再用 `job_output` 收一次；抽查计数为 0 就是改动没进产物，脚本会直接报 FAIL。
...
## 附：成本自查（可选，回答「为什么花了这么多 token」时用）
