...
export const zh = {
  ...
  'action.refresh': '刷新',
  'info.aria': '花费说明',
  'info.hint': '仅能预估 DeepSeek 及 MiMo 相关模型。本会话花费按每条消息的发生时刻（北京时间）所在峰谷时段单价计费：缓存命中输入、未命中输入（含缓存写入）、输出（含推理）分别计价。基础费率按 DeepSeek 8 月 17 日实行的标准，其中 V4 Flash 系列自 9 月 10 日 12:00 起执行新价（谷时 0.02 / 1.0 / 4.0 元每百万 token，峰时为其两倍），该时刻之前的消息仍按原价；高峰时段为北京时间周一至周五 9:00–12:00、14:00–18:00，周末全天按低谷价计费；MiMo 系列不区分峰谷，统一费率。',
...
export const en: Record<BillingKey, string> = {
  ...
  'info.hint': "Only DeepSeek and MiMo models are estimated. This session's spend is priced per message at the rate of its Beijing-time peak/off-peak hour: cache-hit input, cache-miss input (including cache writes), and output (including reasoning) are billed separately. The base rates follow DeepSeek's August 17 pricing, except the V4 Flash series, which was re-priced from September 10, 12:00 Beijing time (off-peak 0.02 / 1.0 / 4.0 CNY per 1M tokens, peak at twice those prices) while messages from before that instant keep the earlier rates; peak hours are Beijing time Monday–Friday 09:00–12:00 and 14:00–18:00, and weekends are billed at off-peak prices all day. MiMo series uses a flat rate with no peak/off-peak distinction.",
