...
- **有费率行才精确计价** —— 会话花费与今日花费优先按价目表（`billing.models`）里该模型自己的行计价；未列出的模型退而按同一家族最接近的行估算并计入（同时标记 `estimated`），连家族都不匹配时才完全不计价，但会把用量记为 `unpriced` 而非无声丢弃。`assistant/attempt` 用最近一条 `request/header` 的模型计价，因此首条 header 之前的 attempt 不计入。
